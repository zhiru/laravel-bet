...
{
    class ShopsController extends \Aireset\Http\Controllers\Controller
    {
...
/*            $checked = new \Aireset\Lib\LicenseDK();
            ...
            $shops = \Aireset\Shop::select('shops.*', 'shops.id AS shop_id');
            ...
            {
                $stats['agents'] = \Aireset\User::where('role_id', 5)->count();
                $stats['distributors'] = \Aireset\User::where('role_id', 4)->count();
            }
            if( auth()->user()->hasRole('agent') )
            {
                $stats['distributors'] = \Aireset\User::where([
                    'role_id' => 4,
            ...
            $agents = \Aireset\User::where('role_id', 5)->pluck('username', 'id')->toArray();
            $distributors = [];
            if( auth()->user()->hasRole(['admin']) )
            {
                $distributors = \Aireset\User::where('role_id', 4)->pluck('username', 'id')->toArray();
            }
        ...
        public function create()
        {
            ...
            $shop = new \Aireset\Shop();
            $availibleUsers = [];
            if( auth()->user()->hasRole('admin') )
            {
                $me = \Aireset\User::where('id', \Auth::id())->get();
                $availibleUsers = \Aireset\User::whereIn('role_id', [
                    4,
                    5
                ])->has('rel_shops')->get();
            ...
            if( auth()->user()->hasRole('distributor') )
            {
                $availibleUsers = \Aireset\User::where('id', \Auth::id())->has('rel_shops')->get();
            }
            $blocks = [];
        ...
        {
            if( $this->max_shops <= \Aireset\Shop::count() )
            {
            ...
            $validatedData = $request->validate([
                ...
                'currency' => 'present|in:' . implode(',', \Aireset\Shop::$values['currency']),
                'percent' => 'required|in:' . implode(',', \Aireset\Shop::$values['percent']),
                'orderby' => 'required|in:' . implode(',', \Aireset\Shop::$values['orderby'])
            ]);
            $data = $request->all();
            $shop = \Aireset\Shop::create($data + ['user_id' => auth()->user()->id]);
            $user = \Aireset\User::find(\Auth::id());
            if( isset($request->categories) && count($request->categories) )
            {
                foreach( $request->categories as $category )
                {
                    \Aireset\ShopCategory::create([
                        'shop_id' => $shop->id,
...
//            $managerBalance = floatval($manager['balance']);
            ...
            $agent = \Aireset\User::create($agent + [
                'parent_id' => auth()->user()->id,
                'role_id' => 5
            ]);
            $agent->attachRole($roles->find(5));
            $distributor = \Aireset\User::create($distributor + [
                'parent_id' => $agent->id,
                'role_id' => 4
            ]);
            $distributor->attachRole($roles->find(4));
            $manager = \Aireset\User::create($manager + [
                'parent_id' => $distributor->id,
                'role_id' => 3
            ]);
            $manager->attachRole($roles->find(3));
            $cashier = \Aireset\User::create($cashier + [
                'parent_id' => $manager->id,
                'role_id' => 2
            ]);
            $cashier->attachRole($roles->find(2));
            $shop = \Aireset\Shop::create($shop + ['user_id' => $distributor->id]);
            $open_shift = \Aireset\OpenShift::create([
            ...
            for( $i = 0; $i < $users['count']; $i++ )
            {
                ...
                $newUser = \Aireset\User::create($data);
        ...
        {
            $shop = \Aireset\Shop::where('id', $shop)->first();
            if( !$shop )
            {
            ...
            {
                $ids = \Aireset\ShopUser::where('user_id', \Auth::id())->pluck('shop_id')->toArray();
            ...
            $cats = \Aireset\ShopCategory::where('shop_id', $shop->id)->pluck('category_id')->toArray();
            $blocks = [];
            if( auth()->user()->hasPermission('shops.unblock') )
            {
                $blocks[0] = __('app.unblock');
        ...
        public function update(\Illuminate\Http\Request $request, \Aireset\Repositories\Session\SessionRepository $sessionRepository, \Aireset\Shop $shop)
        {
            if( \Auth::user()->hasRole([
                'agent',
                'distributor',
            ...
            if( isset($request->categories) && count($request->categories) )
            {
                \Aireset\ShopCategory::where('shop_id', $shop->id)->delete();
                foreach( $request->categories as $category )
                {
                    \Aireset\ShopCategory::create([
                        'shop_id' => $shop->id,
            ...
            {
                $users = \Aireset\User::where('shop_id', $shop->id)->whereIn('role_id', [
                    1,
                    2,
                    3
                ])->get();
            ...
            event(new \Aireset\Events\Shop\ShopEdited($shop));
            return redirect()->route('backend.shop.list')->withSuccess(trans('app.shop_updated'));
        }
        public function delete($shop)
        {
            $usersWithBalance = \Aireset\User::where('shop_id', $shop)->where('role_id', 1)->where('balance', '>', 0)->count();
            if( $usersWithBalance )
            {
                return redirect()->route('backend.shop.list')->withErrors([trans('app.users_with_balance', ['count' => $usersWithBalance])]);
            }
            $gamesWithBalance = \Aireset\GameBank::where('shop_id', $shop)->where(function($query)
            {
            ...
            $jackpotsWithBalance = \Aireset\JPG::where('shop_id', $shop)->where('balance', '>', 0)->count();
            ...
            $pincodesWithBalance = \Aireset\Pincode::where('shop_id', $shop)->where('nominal', '>', 0)->count();
            ...
            $shopInfo = \Aireset\Shop::find($shop);
            if( $shopInfo && $shopInfo->balance > 0 )
            {
                return redirect()->route('backend.shop.list')->withErrors([trans('app.shop_balance')]);
            }
            $distributors = \Aireset\User::where('role_id', 4)->whereHas('rel_shops', function($query) use ($shop)
            {
                $query->where('shop_id', $shop);
            })->pluck('id')->toArray();
            if( count($distributors) )
            {
                $distributorsWithBalance = \Aireset\User::whereIn('id', $distributors)->where('balance', '>', 0)->get();
            ...
            \Aireset\Shop::where('id', $shop)->delete();
            \Aireset\ShopUser::where('shop_id', $shop)->delete();
            \Aireset\Task::create([
                'category' => 'shop',
                'action' => 'delete',
                'item_id' => $shop
        ...
        public function balance(\Illuminate\Http\Request $request)
        {
            ...
            $shop = \Aireset\Shop::find($request->shop_id);
            $user = \Aireset\User::find(\Auth::id());
        ...
        public function action(\Aireset\Shop $shop, $action)
        {
            $open_shift = \Aireset\OpenShift::where([
                'shop_id' => $shop->id,
                'end_date' => null
            ...
            ]) )
            {
                ...
                {
                    case 'jpg_out':
                        $jackpots = \Aireset\JPG::where('shop_id', $shop->id)->get();
                    ...
                    case 'games_out':
                        ...
                        $games = \Aireset\Game::where('shop_id', $shop->id)->get();
                        ...
                        break;
                    case 'return_out':
                        \Aireset\User::where('shop_id', $shop->id)->update(['count_return' => 0]);
                        return redirect()->back()->withSuccess(trans('app.balance_updated'));
                        break;
                }
            }
