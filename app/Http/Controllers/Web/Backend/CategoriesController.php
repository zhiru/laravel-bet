...
{
    class CategoriesController extends \Aireset\Http\Controllers\Controller
    {
...
/*            $checked = new \Aireset\Lib\LicenseDK();
        ...
        public function update(\Aireset\Category $category, \Illuminate\Http\Request $request)
        {
            if( !in_array($category->shop_id, auth()->user()->availableShops()) )
            {
                return redirect()->route('backend.category.list')->withErrors([trans('app.wrong_shop')]);
            ...
            \Aireset\Category::where('id', $category->id)->update($data);
            return redirect()->route('backend.category.list')->withSuccess(trans('app.category_updated'));
        }
        public function delete(\Aireset\Category $category)
        {
            if( !in_array($category->shop_id, auth()->user()->availableShops()) )
            {
                return redirect()->back()->withErrors([trans('app.wrong_shop')]);
            }
            \Aireset\GameCategory::where('category_id', $category->id)->delete();
            $category = \Aireset\Category::where('id', $category->id)->delete();
