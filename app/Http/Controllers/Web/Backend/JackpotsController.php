...
{
    class JackpotsController extends \Aireset\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request)
        {
            ...
            $jackpots = \Aireset\Jackpot::where('shop_id', \Auth::user()->shop_id)->get();
        ...
        {
            $request->validate(['percent' => 'required|in:' . implode(',', \Aireset\Jackpot::$values['percent'])]);
            $data = $request->all();
            ...
            $jackpot = \Aireset\Jackpot::create($data);
            event(new \Aireset\Events\Jackpot\NewJackpot($jackpot));
            return redirect()->route('backend.jackpot.list')->withSuccess(trans('app.jackpot_created'));
        }
        public function edit($jackpot)
        {
            $jackpot = \Aireset\Jackpot::where('id', $jackpot)->first();
        ...
        public function update(\Illuminate\Http\Request $request, \Aireset\Jackpot $jackpot)
        {
            $request->validate(['percent' => 'required|in:' . implode(',', \Aireset\Jackpot::$values['percent'])]);
            $data = $request->only([
                'name',
            ...
            $jackpot->update($data);
            event(new \Aireset\Events\Jackpot\JackpotEdited($jackpot));
            return redirect()->route('backend.jackpot.list')->withSuccess(trans('app.jackpot_updated'));
        }
        public function delete(\Aireset\Jackpot $jackpot)
        {
            if( $jackpot->balance > 0 )
            {
                return redirect()->route('backend.jackpot.list')->withErrors([trans('app.balance_not_zero')]);
            }
            event(new \Aireset\Events\Jackpot\DeleteJackpot($jackpot));
            \Aireset\Jackpot::where('id', $jackpot->id)->delete();
        ...
        public function balance(\Illuminate\Http\Request $request)
        {
            ...
            $jackpot = \Aireset\Jackpot::find($request->jackpot_id);
