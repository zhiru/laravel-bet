...
{
    class SettingsController extends \Aireset\Http\Controllers\Controller
    {
        ...
        public function update(\Illuminate\Http\Request $request, \Aireset\Repositories\Session\SessionRepository $sessionRepository)
        {
            $this->updateSettings($request->except('_token'));
            if( $request->siteisclosed )
            {
                $users = \Aireset\User::where('role_id', '!=', 6)->get();
        ...
        private function updateSettings($input)
        {
            ...
            \Settings::save();
            event(new \Aireset\Events\Settings\Updated());
        }
        public function generator(\Illuminate\Http\Request $request)
        {
            ...
            $games = \Aireset\Game::where('shop_id', \Auth::user()->shop_id)->get()->pluck('name');
            $jackpots = \Aireset\JPG::where('shop_id', \Auth::user()->shop_id)->get()->pluck('name', 'id');
            ...
            if( count($shops) )
            {
                ...
                $apis = \Aireset\Api::where('shop_id', $rarr[0])->get();
                foreach( $apis as $key )
                {
        ...
        public function sync()
        {
            ...
            \Aireset\Jobs\StartSync::dispatch();
            return redirect()->route('backend.settings.general')->withSuccess(trans('app.games_sync_started'));
        }
        public function shop_block(\Illuminate\Http\Request $request, \Aireset\Repositories\Session\SessionRepository $sessionRepository)
        {
