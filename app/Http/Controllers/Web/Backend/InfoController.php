...
{
    class InfoController extends \Aireset\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request)
        {
            ...
            $info = \Aireset\Info::get();
            if( !auth()->user()->hasRole('admin') )
            {
                $info = \Aireset\Info::where('user_id', \Auth::id())->get();
            }
        ...
        public function store(\Illuminate\Http\Request $request)
        {
            ...
            $info = \Aireset\Info::create($data + ['user_id' => \Auth::id()]);
        ...
        public function edit(\Aireset\Info $info)
        {
            if( !$info )
            {
        ...
        public function update(\Illuminate\Http\Request $request, \Aireset\Info $info)
        {
            $data = $request->only([
                'link',
                'title',
            ...
            \Aireset\Info::where('id', $info->id)->update($data);
            \Aireset\InfoShop::where(['info_id' => $info->id])->delete();
            if( $request->shops && count($request->shops) )
            {
                foreach( $request->shops as $shop )
                {
        ...
        public function delete(\Aireset\Info $info)
        {
            \Aireset\Info::where('id', $info->id)->delete();
