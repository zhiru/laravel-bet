...
{
    class PincodeController extends \Aireset\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request)
        {
            ...
            $pincodes = \Aireset\Pincode::select('pincodes.*')->where('pincodes.shop_id', \Auth::user()->shop_id);
            if( $request->download )
            {
            ...
            if( $request->download )
            {
                ...
                $downloader = new \Aireset\Lib\Downloader();
            ...
            if( isset($request->count) && is_numeric($request->count) && isset($request->nominal) && is_numeric($request->nominal) )
            {
                ...
                for( $i = 0; $i < $request->count; $i++ )
                {
                    ...
                    while( $nonUniq )
                    {
                        ...
                        $nonUniq = \Aireset\Pincode::where('code', $pincode)->count();
                    }
                    $data = [
                        'code' => $pincode,
        ...
        public function update(\Illuminate\Http\Request $request, \Aireset\Pincode $pincode)
        {
            ...
            \Aireset\Pincode::where('id', $pincode->id)->update($data);
            return redirect()->route('backend.pincode.list')->withSuccess(trans('app.pincode_updated'));
        }
        public function delete(\Aireset\Pincode $pincode)
        {
            if( !in_array($pincode->shop_id, auth()->user()->availableShops()) )
            {
                return redirect()->back()->withErrors([trans('app.wrong_shop')]);
