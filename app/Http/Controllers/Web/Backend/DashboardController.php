...
{
    class DashboardController extends \Aireset\Http\Controllers\Controller
    {
        private $users = null;
...
/*            $checked = new \Aireset\Lib\LicenseDK();
            ...
            $stats = [
                ...
                'banned' => $this->users->countByStatus(\Aireset\Support\Enum\UserStatus::BANNED, $ids),
                'games' => \Aireset\Game::where([
            ...
            $user = \Auth::user();
            $shops = \Aireset\Shop::orderBy('id', 'desc')->whereIn('id', $availableShops)->take(5)->get();
            $summ = \Aireset\User::where([
                'shop_id' => \Auth::user()->shop_id,
                'role_id' => 1
            ])->sum('balance');
            $open_shift = \Aireset\OpenShift::select('open_shift.*')->whereIn('open_shift.shop_id', $availableShops)->orderBy('open_shift.start_date', 'DESC')->take(5)->get();
            $statistics = \Aireset\Transaction::whereIn('user_id', $ids)->orderBy('id', 'desc')->take(5)->get();
            $gamestat = \Aireset\StatGame::whereIn('user_id', $ids)->orderBy('date_time', 'DESC')->take(5)->get();
            $bank_stat = \Aireset\BankStat::whereIn('user_id', $ids)->orderBy('created_at', 'DESC')->take(5)->get();
            $shops_stat = \Aireset\ShopStat::whereIn('user_id', $ids)->orderBy('date_time', 'DESC')->take(5)->get();
        ...
        public function live_stat(\Illuminate\Http\Request $request)
        {
            ...
            $transactions = \Aireset\Transaction::select('transactions.*')->orderBy('transactions.created_at', 'DESC');
            ...
            $statgames = \Aireset\StatGame::select('stat_game.*')->whereIn('stat_game.shop_id', $availableShops)->take(100)->orderBy('id', 'DESC');
            ...
            $bankstat = \Aireset\BankStat::select('bank_stat.*')->whereIn('bank_stat.shop_id', $availableShops)->take(100)->orderBy('id', 'DESC');
            ...
            $ShopStat = \Aireset\ShopStat::select('shops_stat.*')->whereIn('shops_stat.shop_id', $availableShops)->take(100)->orderBy('id', 'DESC');
        ...
        public function start_shift(\Illuminate\Http\Request $request)
        {
            ...
            $shop = \Aireset\Shop::find(\Auth::user()->shop_id);
            if( !$shop )
            {
        ...
        {
            $users = auth()->user()->hierarchyUsers();
            $statistics = \Aireset\Transaction::select('transactions.*')->orderBy('transactions.created_at', 'DESC');
            ...
            if( $request->shifts != '' )
            {
                $shift = \Aireset\OpenShift::find($request->shifts);
                if( $shift )
                {
            ...
            $statistics = \Aireset\ShopStat::select('shops_stat.*', 'shops_stat.date_time AS created_at')->whereIn('shops_stat.shop_id', auth()->user()->availableShops());
            if( $request->system_str != '' )
            {
        ...
        {
            $users = auth()->user()->hierarchyUsers();
            $statistics = \Aireset\StatGame::select('stat_game.*')->whereIn('stat_game.shop_id', auth()->user()->availableShops())->orderBy('stat_game.date_time', 'DESC');
        ...
        {
            \Aireset\StatGame::where('shop_id', \Auth::user()->shop_id)->delete();
            \Aireset\GameLog::where('shop_id', \Auth::user()->shop_id)->delete();
            \Aireset\Game::where('shop_id', \Auth::user()->shop_id)->update([
                'stat_in' => 0,
                'stat_out' => 0
            ]);
        ...
        {
            $users = auth()->user()->hierarchyUsers();
            $statistics = \Aireset\BankStat::select('bank_stat.*')->whereIn('bank_stat.shop_id', auth()->user()->availableShops())->orderBy('bank_stat.created_at', 'DESC');
        ...
        public function shop_stat(\Illuminate\Http\Request $request)
        {
            ...
            {
                $ids = \Aireset\ShopUser::where('user_id', \Auth::id())->pluck('shop_id');
                if( count($ids) )
                {
                    $shops = \Aireset\Shop::whereIn('id', $ids)->pluck('name', 'id');
                }
            }
            $statistics = \Aireset\ShopStat::select('shops_stat.*')->whereIn('shops_stat.shop_id', auth()->user()->availableShops())->orderBy('shops_stat.date_time', 'DESC');
            if( $request->name != '' )
            {
                $statistics = $statistics->join('shops', 'shops.id', '=', 'shops_stat.shop_id');
        ...
        public function search(\Illuminate\Http\Request $request)
        {
            ...
            $users = \Aireset\User::whereIn('id', $hierarchyUsers)->where(function($q) use ($query)
            {
                $q->where('email', 'like', '%' . $query . '%')->orWhere('username', 'like', '%' . $query . '%');
            })->orderBy('created_at', 'DESC')->take(25)->get();
            $pay_stats = \Aireset\Transaction::select('*')->whereIn('shop_id', $availableShops)->orderBy('created_at', 'DESC')->whereIn('user_id', $hierarchyUsers)->where(function($q) use ($query)
            {
            ...
            $game_stats = \Aireset\StatGame::select('stat_game.*')->whereIn('stat_game.shop_id', $availableShops)->orderBy('stat_game.date_time', 'DESC')->whereIn('stat_game.user_id', $hierarchyUsers)->where(function($q) use ($query)
            {
            ...
            $bank_stats = \Aireset\BankStat::select('*')->whereIn('shop_id', $availableShops)->orderBy('created_at', 'DESC')->whereIn('user_id', $hierarchyUsers)->where(function($q) use ($query)
            {
            ...
            $shop_stats = \Aireset\ShopStat::select('*')->whereIn('shop_id', $availableShops)->orderBy('date_time', 'DESC')->whereIn('user_id', $hierarchyUsers)->where(function($q) use ($query)
            {
            ...
            $shift_stats = \Aireset\OpenShift::select('*')->whereIn('shop_id', $availableShops)->orderBy('start_date', 'DESC')->whereIn('user_id', $hierarchyUsers)->where(function($q) use ($query)
            {
                $q->whereHas('user', function($q2) use ($query)
                {
                    $q2->where('username', 'like', '%' . $query . '%')->orWhere('email', 'like', '%' . $query . '%');
        ...
        public function shift_stat(\Illuminate\Http\Request $request)
        {
            ...
            $statistics = \Aireset\OpenShift::select('open_shift.*')->whereIn('open_shift.shop_id', auth()->user()->availableShops())->orderBy('open_shift.start_date', 'DESC');
            if( $request->shifts != '' )
            {
