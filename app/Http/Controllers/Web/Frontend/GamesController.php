...
/*            $checked = new \Aireset\Lib\LicenseDK();
            ...
            $shop = \Aireset\Shop::find($shop_id);
            ...
            if( $category1 == '' )
            {
                ...
                {
                    $category = \Aireset\Category::where([
            ...
            $games = $games->get();
            $jpgs = \Aireset\JPG::where('shop_id', $shop_id)->get();
            $categories = false;
            $currentSliderNum = -1;
            if( $games )
            {
                $cat_ids = \Aireset\GameCategory::whereIn('game_id', \Aireset\Game::where([
                    'view' => 1,
                    'shop_id' => $shop_id
                ])->pluck('id'))->groupBy('category_id')->pluck('category_id');
                if( count($cat_ids) )
                {
                    $categories = \Aireset\Category::whereIn('id', $cat_ids)->get();
                    if( $category1 != '' )
                    {
        ...
        public function search(\Illuminate\Http\Request $request)
        {
            ...
            $games = \Aireset\Game::where('view', 1);
