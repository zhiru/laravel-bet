...
{
    class ProfileController extends \Aireset\Http\Controllers\Controller
    {
        ...
        public function index(\Aireset\Repositories\Role\RoleRepository $rolesRepo)
        {
            $user = $this->theUser;
            $edit = true;
        ...
        public function updateDetails(\Aireset\Http\Requests\User\UpdateProfileDetailsRequest $request)
        {
        ...
        public function updatePassword(\Aireset\Http\Requests\User\UpdateProfilePasswordRequest $request)
        {
        ...
        public function updateAvatar(\Illuminate\Http\Request $request, \Aireset\Services\Upload\UserAvatarManager $avatarManager)
        {
        ...
        public function updateAvatarExternal(\Illuminate\Http\Request $request, \Aireset\Services\Upload\UserAvatarManager $avatarManager)
        {
            $avatarManager->deleteAvatarIfUploaded($this->theUser);
            return $this->handleAvatarUpdate($request->get('url'));
        }
        public function updateLoginDetails(\Aireset\Http\Requests\User\UpdateProfileLoginDetailsRequest $request)
        {
        ...
        public function activity(\Aireset\Repositories\Activity\ActivityRepository $activitiesRepo, \Illuminate\Http\Request $request)
        {
            $user = $this->theUser;
        ...
        public function sessions(\Aireset\Repositories\Session\SessionRepository $sessionRepository)
        {
            $profile = true;
            $user = $this->theUser;
        ...
        public function invalidateSession($session, \Aireset\Repositories\Session\SessionRepository $sessionRepository)
        {
        ...
        {
            $history = \Aireset\Payment::where('user_id', \Auth::user()->id)->orderBy('created_at', 'DESC')->paginate(25);
        ...
        public function balanceAdd(\Illuminate\Http\Request $request)
        {
            ...
            {
                $payment = \Aireset\Payment::create([
            ...
            if( $request->system == 'coinpayment' )
            {
                ...
                $payment = \Aireset\Transaction::create([
                    'user_id' => \Auth::user()->id,
                    'summ' => abs($amount),
                    'system' => $request->system,
                    'shop_id' => \Auth::user()->shop_id,
        ...
        public function pincode(\Illuminate\Http\Request $request)
        {
            ...
            $user->update([
                ...
                'count_return' => $user->count_return + \Aireset\Lib\Functions::count_return($pincode->nominal, $user->shop_id),
        ...
        public function returns(\Illuminate\Http\Request $request)
        {
            ...
            $return = \Aireset\Returns::where('shop_id', $user->shop_id)->first();
            if( $sum )
            {
