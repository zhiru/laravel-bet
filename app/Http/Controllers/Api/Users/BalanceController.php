...
{
    class BalanceController extends \Aireset\Http\Controllers\Api\ApiController
    {
        ...
        public function balance(\Illuminate\Http\Request $request, \Aireset\User $user, $type = 'add')
        {
            if( !in_array($type, [
                'add',
                'out'
            ...
            if( $user->hasRole('user') )
            {
                ...
                {
                    $user->update(['count_return' => $user->count_return + \Aireset\Lib\Functions::count_return($sum, $user->shop_id)]);
                }
            }
            if( auth()->user()->hasRole('cashier') && $user->hasRole('user') )
            {
